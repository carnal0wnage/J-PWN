...
def check_open_servicedesk_signup(url):
    ...
    '''
    service_desk_url = f"{url.rstrip('/')}/servicedesk/customer/user/signup"
    vulnerabilities = ''
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            vulnerabilities += (f"+ [Initial Access] Open Service Desk Signup Found: Manual exploitation required [try to signup and log in] | URL: {service_desk_url}")
            print(f"\n{Fore.GREEN}+ [Initial Access] Open Service Desk Signup Found: Manual exploitation required [try to signup and log in]{Style.RESET_ALL}")
            ...
            print(f"{Fore.YELLOW}\n- No Open Service Desk Signup detected on: {service_desk_url}{Style.RESET_ALL}")
