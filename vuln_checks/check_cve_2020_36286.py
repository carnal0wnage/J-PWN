...
def check_cve_2020_36286(url):
    """
    Checks for Unauthenticated Issues with content vulnerability in Jira and parses the response.
    """
    check_unauthenticated_issues_url = f"{url.rstrip('/')}/rest/api/2/search?jql=assignee%20in%20(membersOf(\"jira-users\"))"
    vulnerabilities = ''

    try:
        print(f"{Fore.YELLOW}\nINFO: IN DEVELOPMENT Checking for Unauthenticated Assignee in membersOf{Style.RESET_ALL}")
